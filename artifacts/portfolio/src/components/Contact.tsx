...
export function Contact() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement>) => {
    ...
    if (errors[e.target.name as keyof typeof errors]) setErrors((p) => ({ ...p, [e.target.name]: undefined }));
  };
    ...
    <section id="contact" className="relative py-28 overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <h2 className="text-4xl md:text-5xl font-poppins font-extrabold text-white mb-4">
        ...
        <div className="grid lg:grid-cols-5 gap-8 max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <div className="glass-panel rounded-2xl p-7 border-white/[0.07]">
            ...
            <div className="glass-panel rounded-2xl p-7 border-white/[0.07]">
              ...
              <div className="flex flex-col gap-3">
                <a href={PORTFOLIO_DATA.personal.social.linkedin} target="_blank" rel="noreferrer"
                  className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-blue-600/10 border border-blue-600/20 text-white/75 hover:bg-blue-600/20 hover:text-white transition-all">
                  <FaLinkedin size={17} className="text-blue-400" /><span className="text-sm font-medium">LinkedIn</span>
                </a>
                <a href={PORTFOLIO_DATA.personal.social.github} target="_blank" rel="noreferrer"
                  className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-white/75 hover:bg-white/10 hover:text-white transition-all">
                  <FaGithub size={17} className="text-white/60" /><span className="text-sm font-medium">GitHub</span>
                </a>
                <a href={PORTFOLIO_DATA.personal.resumeLink} target="_blank" rel="noreferrer"
                  className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-gradient-to-r from-violet-500/15 to-cyan-500/15 border border-violet-500/25 text-white/75 hover:opacity-90 transition-all">
                  <Download size={17} className="text-cyan-400" /><span className="text-sm font-medium">Download Resume</span>
                </a>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <div className="glass-panel rounded-2xl p-8 md:p-10 h-full flex flex-col border-white/[0.07]">
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-5 flex-1">
                <div className="grid sm:grid-cols-2 gap-5">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input name="from_name" value={form.from_name} onChange={handleChange} placeholder="Full name"
                      className={`bg-white/[0.04] border rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:bg-white/[0.07] transition-all ${errors.from_name ? "border-rose-500/50" : "border-white/10 focus:border-violet-500/50"}`} />
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input name="reply_to" type="email" value={form.reply_to} onChange={handleChange} placeholder="your@email.com"
                      className={`bg-white/[0.04] border rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:bg-white/[0.07] transition-all ${errors.reply_to ? "border-rose-500/50" : "border-white/10 focus:border-violet-500/50"}`} />
                ...
                <div className="flex flex-col gap-1.5 flex-1">
                  ...
                  <textarea name="message" value={form.message} onChange={handleChange}
                    placeholder="Tell me about the opportunity, project, or idea..." rows={7}
                    className={`bg-white/[0.04] border rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:bg-white/[0.07] transition-all resize-none ${errors.message ? "border-rose-500/50" : "border-white/10 focus:border-violet-500/50"}`} />
                  {errors.message && <span className="text-xs text-rose-400">{errors.message}</span>}
                </div>
                <button type="submit" disabled={status === "sending"}
                  className="flex items-center justify-center gap-2.5 px-6 py-4 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 text-white font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 shadow-lg shadow-violet-500/20 text-sm">
                  {status === "sending"
